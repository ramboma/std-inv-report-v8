...
def employability_college(data):
    ...
    # step2:B9-1回答"无法评价"人数
    answer_except_count = answerUtil.answer_of_subject_count_grp(data_where, [BASE_COLUMN[0], subject], BASE_COLUMN[0],
...
def employability_major(data):
    ...
    # step1:各学院 B9-1答题总人数
    answer_count = answerUtil.answer_grp_count(data_where, BASE_COLUMN[0:1].append(subject), BASE_COLUMN[0:1], subject)
    print("各学院专业{}回答总人数：\n".format(subject))
    print(answer_count)

    # step2:B9-1回答"无法评价"人数
    answer_except_count = answerUtil.answer_of_subject_count_grp(data_where, BASE_COLUMN[0:1].append(subject),
                                                                 BASE_COLUMN[0:1], subject, ANSWER_NORMAL_1[-1])
    print("各学院{}回答'{}'人数：\n".format(subject, ANSWER_NORMAL_1[-1]))
    print(answer_except_count)
    ...
    if answer_except_count.empty:
        ...
        print('need except')
        pd_left = pd.merge(answer_count, answer_except_count, how='left',
                           on=BASE_COLUMN[0:1], validate='one_to_one')
        pd_left.fillna(0)
        pd_left['count'] = pd_left['B9-1-x'] - pd_left['B9-1-y']
        pd_five_total = pd.DataFrame({'college': pd_left[BASE_COLUMN[0]],
    ...
    # step3:B9-1各答案人数
    answer_values_grp = answerUtil.answer_grp_count(data_where, BASE_COLUMN[0:1].append(subject,'A2'),
                                                    BASE_COLUMN[0:1].append(subject), subject)
    print("{}各答案分布：".format(subject))
    print(answer_values_grp)

    # step4:B9-1五维占比
    pd_five = pd.merge(answer_values_grp, pd_five_total, how='inner',
                       left_on='_10,_14', right_on='college,major', validate='many_to_one')
    pd_five['rate'] = (pd_five['A2'] / pd_five['count'] * 100).round(decimals=2)

    pd_five_rate = pd.DataFrame(pd_five, columns=['_10', '_14', 'B9-1', 'rate'])
    print("各学院{}五维占比：\n".format(subject))
    print(pd_five_rate)
